/* EuserChat Widget v3.0 | (c) 2026 Euser AI | euserai.com */
...
(function(){
  ...
  function md(t){
    ...
    /* Custom buttons */
    t=t.replace(/\[BTN:([^\]]+)\]\(([^)]+)\)/gi,function(_,label,url){
      return '<a href="'+url+'" target="_blank" rel="noopener noreferrer" style="display:inline-block; margin:6px 0; padding:10px 16px; background:linear-gradient(135deg,#D4AF37,#e8c44a); color:#000; font-size:13px; font-weight:600; text-decoration:none; border-radius:12px; box-shadow:0 2px 10px rgba(212,175,55,0.25); text-align:center;">'+label+'</a>';
    });
    /* Regular links */
  ...
  function send(){
    ...
    .then(function(r){
      console.log("EuserChat FETCH RESPONSE:", r.status, r.statusText);
      if(!r.ok) throw new Error("n8n HTTP Error " + r.status + ": " + r.statusText);
      return r.text();
    })
    .then(function(raw){
      console.log("EuserChat RAW TEXT:", raw);
      typEl.classList.remove("on");sendB.disabled=false;inp.focus();
      var reply=raw;
      try{
        var d=JSON.parse(raw);
        console.log("EuserChat JSON PARSED:", d);
        if(Array.isArray(d))d=d[0]||{};
        reply=d.output||d.response||d.text||d.message||raw;
        console.log("EuserChat REPLY EXTRACTED:", reply, "Type:", typeof reply);
      }catch(e){
        console.log("EuserChat JSON PARSE FAILED:", e);
      }
    ...
    .catch(function(err){
      console.error("EuserChat CAUGHT ERROR:", err);
      typEl.classList.remove("on");sendB.disabled=false;
      addMsg("ai", "System Error: " + err.message + ". Check console for details.");
    });
  }
